#!/bin/sh
set -eu
version=${WARDEN_VERSION:-latest}; output=warden; force=false
...
if [ "$version" = latest ]; then version=$(curl --proto '=https' --tlsv1.2 -fsSL https://api.github.com/repos/warden-cv/warden/releases/latest | sed -n 's/.*"tag_name":[[:space:]]*"\([^"]*\)".*/\1/p' | head -1); fi
[ -n "$version" ] || { echo 'Could not resolve the latest Warden release.' >&2; exit 1; }
if [ -e "$output" ] || [ -L "$output" ]; then $force || { echo "Refusing to overwrite $output; pass --force." >&2; exit 1; }; fi
asset="warden-${os}-${arch}.tar.gz"; base="https://github.com/warden-cv/warden/releases/download/$version"; tmp=$(mktemp -d); trap 'rm -rf "$tmp"' EXIT INT TERM
...
[ "$actual" = "$expected" ] || { echo 'Checksum mismatch.' >&2; exit 1; }; tar -xzf "$tmp/$asset" -C "$tmp" warden; chmod 0755 "$tmp/warden"; mv "$tmp/warden" "$output"
echo "Downloaded Warden $version to $output"
